function tempW(filespec, k, th)
    ...
    for i=1:length(files)
        ...
        [P3 E3 F3 D3] = TriangulationCarving(P2, F2, Sal(:,3), 1.0e-3);
        [D dvals] = TriangulationDistanceMap(P3, F3(find(F3(:,4)),:), size(em));
        [seeds] = TriangulationSegmentationSeeds(D, 10.0, .5);
        
        P(end+1:end+size(seeds,1),1:2) = seeds(:,1:2);
        [P F E] = DotOperations('delaunay', P);
        [P2 E2 F2 Sal] = TriangulationEdgeSaliency(P);
        [P3 E3 F3 D3] = TriangulationCarving(P2, F2, Sal(:,3), 1.0e-3);        
        [U W] = TriangulationEdgeCut(P3, F3, Sal(:,3), seeds);
        figure(1); clf;
        showPoints(P, [], U(:,3), 'MarkerSize', 20);
        figure(2); clf;
        tempDrawEdges(P2, W, W(:,3), 0, 'LineWidth', 2);
        hold on;
        plot(seeds(:,2), seeds(:,1), 'x', 'LineWidth', 2, 'Color', [0 0 0], 'MarkerSize', 20);
        figure(3); clf;
        tempDrawTriangles2(P3, F3(find(F3(:,4)),:));
        figure(4); clf;
        myimage(D); axis image;
        figure(5); clf;
        tempDrawEdges(P2, E2, Sal(:,3), 0, 'LineWidth', 2);
        fprintf('hit ENTER\n');
        pause;
        clear C C0 em P F E P2 E2 F2 P3 E3 F3 D dvals U W seeds Sal D3;
    end
