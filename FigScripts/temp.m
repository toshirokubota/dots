...
%     load 'C:\Users\kubota\Documents\MATLAB\Fillingin\data\36.mat';
    load 'C:\Data\images\Data01\111B.mat';
    [em2 P0] = randomSubsample(em, 3);
    [P2 F2 E2 S T X] = TriangulationEdgeSaliency(P0, 10, 0., 0, 0.75, 0, 100);
    [P5 E5 F5 D D2 I L] = TriangulationSegmentationSeeds(P2, F2, E2(:,6)>4, 5, 5);
    starts=find(F5(:,4));
...
%         [P4 E4 F4 O N] = TriangulationSegmentation(P2, F2, (E2(:,5)), start, [size(F,1)-50], [1 10 10 10 .0], [0.0 0.0 15 15]);
        [P4 E4 F4 O N] = TriangulationSegmentation(P2, F2, (E2(:,5)), start, [200], [0 1 10 1 0], [0.5 0.5 10 10]);
%         stops = find(N(:,3));
        stops = find(N(:,3)); 
        [v id] = max(N(stops,4));
        stops = stops(id);
