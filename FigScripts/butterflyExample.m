function temp()
    ...
    load 'C:\Users\kubota\Documents\MATLAB\Fillingin\data\60.mat';    
...
%         [P4 E4 F4 O N] = TriangulationSegmentation(P2, F2, (E2(:,5)), start, [size(F,1)-50], [1 10 10 10 .0], [0.0 0.0 15 15]);
        [P4 E4 F4 O N] = TriangulationSegmentation(P2, F2, (E2(:,5)), start, [100], [0 1 10 1 0], [0.5 0.5 10 10]);
%         stops = find(N(:,3));
% % %         stops = find(N(:,3)); 
% % %         [v id] = max(N(stops,4));
% % %         stops = stops(id);
        [v stops] = max(N(20:end,4));
        stops = stops + 19;
